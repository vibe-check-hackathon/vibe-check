...
const AGENTS = [
  {
    n: "Kestrel",
    r: "Founder interviews",
    status: "Built",
    note: "laura/pipeline/lib/interview-agent.js — ElevenLabs, gated by /integrations",
  },
  {
    n: "Checky",
    r: "Evidence-grounded Q&A",
    status: "Built",
    note: "laura/pipeline/lib/assistant.js — retrieval over the opportunity DB",
  },
  {
    n: "Screening + diligence checks",
    r: "Thesis screen, corroboration, floors",
    status: "Built",
    note: 'deterministic — laura/pipeline/lib/screening.js, corroborate.js (not an LLM "agent")',
  },
  {
    n: "Reference calls (warm or cold)",
    r: "Reference & background checks",
    status: "Planned",
    note: "no code yet — see sun/system-architecture.md §7",
  },
  {
    n: "Deck & data-room extraction",
    r: "OCR / document parsing",
    status: "Planned",
    note: "no code yet — see sun/tech-video-staging/tech-spec.md §3.5",
  },
];
...
export function SettingsPage() {
  ...
  const thesisRows = thesis
    ? [
        ...
        {
          k: "Stages",
          v: `${thesis.fund.stages.join(" · ")} · check ${money(thesis.fund.checkSizeUsd.min)} – ${money(thesis.fund.checkSizeUsd.max)}`,
        },
        { k: "Geography", v: thesis.fund.geographies.join(" · ") },
        { k: "Ownership", v: `${Math.round(thesis.fund.targetOwnership * 100)}% target` },
        {
          k: "Risk appetite",
          v: `${thesis.fund.riskAppetite} · max ${thesis.fund.maxOpenGaps} open gaps at decision`,
        },
      ]
      ...
      <div className="px-8 py-6 grid lg:grid-cols-2 gap-4">
        ...
        <Card className="p-0 lg:col-span-2">
          <div className="px-5 py-3.5 border-b border-border flex items-center justify-between">
            ...
            <Badge tone="teal">
              {AGENTS.filter((a) => a.status === "Built").length} built ·{" "}
              {AGENTS.filter((a) => a.status === "Planned").length} planned
            </Badge>
          </div>
          ...
          <div className="divide-y divide-border">
            {[
              ...
              <div key={n} className="flex items-center gap-3 px-5 py-3">
                <div className="h-8 w-8 rounded-full bg-primary/10 text-primary grid place-items-center text-[11px] font-medium">
                  {i}
                </div>
