---
schema_version: 1
id: OPP-MGV-0002
company: deskbird
status: closed
source_channel: outbound
created_at: 2023-09-05T09:00:00Z
updated_at: 2026-07-19T00:00:00Z
decision_deadline: 2023-09-06T09:00:00Z
founder_ids: [FND-MGV-0003, FND-MGV-0004]
thesis_id: THESIS-001
real_event: "$13M Series A co-led by ALSTIN Capital and AXA Venture Partners, announced 2023-09-06"
data_basis: public sources, compiled 2026-07-19 — retrospective demo card
---

# deskbird

## Summary

**One-line pitch:** Workplace management platform for hybrid work — desk booking
and flexible workspace solutions (St. Gallen, Switzerland).

**Recommendation:** Invest *(real-world outcome: ALSTIN co-led the round)*
**Recommendation confidence:** not assessed — retrospective card

Real deal: $13M Series A co-led by ALSTIN Capital and AXA Venture Partners with
session.vc, Rivus Capital, and PortfoLion [CLM-001][SRC-002]. The company later
raised a $23M Series B on 2025-09-24, led by Octopus Ventures with participation
from Alstin Capital, Neva SGR, AVP, session.vc, and PortfoLion [CLM-004][SRC-007].

## Founders and Team

### FND-MGV-0003 - Ivan Cossu, CEO & co-founder

- **Founder Score snapshot:** not assessed — no fabricated evaluations of real people
- Publicly discussed the 2021 near-bankruptcy pivot from coworking booking to
  hybrid-work management [CLM-003][SRC-003] — exactly the kind of
  adversity-response evidence the interview stage looks for
- Public interview available (see Interview)

### FND-MGV-0004 - Jonas Hess, co-founder

- **Founder Score snapshot:** not assessed
- Co-founder per public company profiles [CLM-005][SRC-005]

## Assessment

| Axis | Rating | Trend | Confidence | Rationale |
| --- | --- | --- | --- | --- |
| Founder | not assessed | — | unknown | No fabricated judgments of real founders |
| Market | not assessed | — | unknown | Would require the research agent run |
| Idea vs. market | not assessed | — | unknown | — |

## Evidence and Gaps

| ID | Claim or gap | State | Trust | Evidence / next action |
| --- | --- | --- | --- | --- |
| CLM-001 | $13M Series A co-led by ALSTIN + AVP, announced 2023-09-06 | verified | 90/100 | [SRC-002][SRC-006] |
| CLM-002 | In ALSTIN active portfolio as of 2026-07-19 | verified | 90/100 | [SRC-001] |
| CLM-003 | 2021 pivot after near-bankruptcy (founder's own account) | claimed | 50/100 | [SRC-003] — self-reported in interview |
| CLM-004 | $23M Series B announced 2025-09-24, with Alstin Capital participating | verified | 90/100 | [SRC-007] |
| CLM-005 | Co-founders: Ivan Cossu, Jonas Hess | verified | 80/100 | [SRC-005] |
| GAP-001 | Current ARR / customer count | open | unknown | Not publicly disclosed |

### Sources

- **SRC-001:** ALSTIN Capital portfolio page, fetched 2026-07-19 — https://alstin.capital/portfolio/
- **SRC-002:** Tech.eu, 2023-09-06 — https://tech.eu/2023/09/06/deskbird-secures-13m-to-enhance-features-with-ai/
- **SRC-003:** AVP "Meet the founder" interview — https://avpcap.com/meet-the-founder-ivan-cossu-from-deskbird/
- **SRC-004:** Startup Insider podcast (2025-10-09) — https://open.spotify.com/episode/1PiZG16jRaQFby2BSUz3rd
- **SRC-005:** Crunchbase person profile — https://www.crunchbase.com/person/ivan-cossu
- **SRC-006:** deskbird press release — https://www.deskbird.com/press-releases/series-a-funding
- **SRC-007:** deskbird Series B press release, 2025-09-24 — https://www.deskbird.com/press-releases/23-million-series-b

## Interview

▶ **Public founder interview found:** [#164 Flexibles Desk Booking mit Ivan Cossu
von deskbird (YouTube, 2024)](https://www.youtube.com/watch?v=Flx_ZmOau68) —
covers the product pivot and hybrid-work market. The Matching stage would mine
this for hypothesis tests (pivot response = resilience evidence); the actual
investor interview remains **🔒 confidential**.

## Decision

- **Real-world outcome:** ALSTIN co-led the $13M Series A (2023-09-06) —
  `outcome.recorded: term_sheet_signed`.
- Internal negotiation model and diligence: **🔒 confidential**.
