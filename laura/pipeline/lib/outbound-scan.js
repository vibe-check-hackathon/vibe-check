...
/** Fill the intelligence-brief template for one scanned record — the card that
 *  lands in opportunity-db, per the template's structure and integrity rules. */
export function briefMarkdown(r) {
  const unverified = r.verification !== "web-searched";
  const founderRows = r.founders
    .map((f) => `| \`${f.id}\` | ${f.name} | ${f.role} | ${f.publicSignal || "—"} |`)
    .join("\n");
  const sourceRows = r.sources.length
    ? r.sources.map((s, i) => `- **SRC-${String(i + 1).padStart(3, "0")}:** ${s.label} — ${s.url}`).join("\n")
    : "- none captured — treat every statement as claimed";
  return `---
schema_version: 1
id: ${r.id}
company: ${JSON.stringify(r.company)}
status: open
source_channel: outbound_live_scan
scan_region: ${r.scanRegion}
verification: ${r.verification}
created_at: ${new Date().toISOString()}
thesis_id: THESIS-001
data_basis: "${unverified ? "UNVERIFIED model recall — no web access; verify every statement before use" : "public sources via live web search; verify before outreach"}"
---
# ${r.company}

> Outbound Person & Potential Team Intelligence Brief — generated by the live
> scan. ${unverified ? "**UNVERIFIED (model recall).** Every claim below is `claimed` until checked." : "Web-sourced; confirm sources before outreach."}
> Founder facts are public-professional evidence only — no personality scoring,
> no sensitive data. Unknown evidence scores zero (unknown ≠ false).

## Executive Summary

**One-line pitch:** ${r.oneLiner}
**Why now:** ${r.outboundRationale}
**Trigger:** ${r.activitySignal}
**Recommended action:** verify sources, then 24-hour founder call if allocation fits.

## Startup Hypothesis and Stage

| Field | Value |
|---|---|
| Company / project | ${r.company} |
| Geography | ${r.location} |
| Product/category | ${r.oneLiner} |
| Estimated stage | ${r.stage} |
| Public funding | ${r.latestRound ?? "unknown"} |

## Founders and Team

| Founder ID | Person | Public role | Evidence-based professional signal |
|---|---|---|---|
${founderRows || "| — | not publicly identified | — | — |"}

## Screening (canonical thesis screen)

- **Pass:** ${r.screening.pass}
- **Soft flags:** ${r.screening.softFlags.length ? r.screening.softFlags.join("; ") : "none"}
